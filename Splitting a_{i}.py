...
SplitNodes = []
### A list of nodes and their articulation point probabilities that will be remove
for i in range(5):
    G.remove_node(max(SliceList, key=itemgetter(0))[1])
    SplitNodes.append(max(SliceList, key=itemgetter(0))[1])
    SliceList.remove(max(SliceList, key=itemgetter(0)))
    ### Removes the node with the largest articulation point probability

print(SplitNodes)
### Lists what nodes were removed

plt.figure(1)
pos1 = nx.spring_layout(G, k=0.97, iterations=80, )#seed=20033)
...
plt.savefig('ER2 N=40 Seed=34 Split (a_{i})', bbox_inches='tight', dpi=300)
plt.show()
